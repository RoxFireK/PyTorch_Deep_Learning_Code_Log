...
class Generator(nn.Module):
    def __init__(self, z_dim, img_dim):                                 
        # z_dim: 将噪声图片拉伸成一维时候的大小
        super().__init__()
        self.gen = nn.Sequential(
            nn.Linear(z_dim, 256),
            nn.LeakyReLU(0.1),
            nn.Linear(256, img_dim),    # 28*28*1---784                 
            # img_dim： 将生成的二维图像拉伸成一维时候的大小
            nn.Tanh(),
        )
...
step = 0
# 训练鉴别器：最大化 log(D(real)) + log(1-D(G(z)))
for epoch in range(num_epochs):
    for real, _ in loader:
        ...
        # 鉴别器损失
        lossD_real = criterion(disc(real), 1)  # 真实图→1
        lossD_fake = criterion(disc(fake), 0)  # 假图→0
        lossD = (lossD_real + lossD_fake) / 2
        
        # 生成器损失：最大化 log(D(G(z)))
        lossG = criterion(disc(fake), 1)  # 欺骗鉴别器
        
        # 反向传播（交替训练）
        lossD.backward()  # 只更新鉴别器
        lossG.backward()  # 只更新生成器
...
for epoch in range(num_epochs):
    for batch_idx, (real, _) in enumerate(loader):                      
        ...
        # a. 计算鉴别器处理的真实影像和 1 的损失值———得log(D(real))

        disc_fake = disc(fake).view(-1)                                 
        # 将噪声影像放入鉴别器中进行处理
        lossD_fake = criterion(disc_fake, torch.zeros_like(disc_fake))  
        # b. 计算鉴别器处理的生成影像和 0 的损失值———得log(1-D(G(z)))
        lossD = (lossD_real + lossD_fake)/2                             
        # 得到鉴别器的损失值（上面讲解会提到）
        disc.zero_grad()                                                
        # 梯度清零
